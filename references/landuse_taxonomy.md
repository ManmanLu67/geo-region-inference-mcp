...
MCP 几何统计的面积与 compactness 均为 **净面积**口径（外环减孔洞、周长仅外环）。带孔地物 compactness 会相应偏低（净/外环约 0.93 时降约 7%）。`analyze_regions` 与调试工具 `calculate_geometry` 共用 fail-fast / `GEOMETRY_INVALID`；`calculate_geometry` 仍不查在线源。

| 大类标签 | 典型面积范围 | 形状信号（来自 MCP 几何统计；调试可用 `calculate_geometry`） | OSM 标签线索 | 常见建筑 | 常见项目命名模式 |
|---|---|---|---|---|---|
...
3. 若完全没有在线数据，用 `properties` 中任何看起来像用地代码/规划编号的字段（即使不认识具体编码规则，也可以结合数字/字母前缀做弱证据），再结合 MCP `analyze_regions` 返回的几何字段（面积、紧凑度、长宽比）做形状匹配。调试可用 `calculate_geometry`（几何校验与主路径相同，但不含 POI）。
