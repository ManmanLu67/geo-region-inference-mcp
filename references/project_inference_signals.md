...
ArcGIS / 环保台账常见编码字段，不要求预先知道编码规则；语义明显相关时应记录原字段名与值：

| 字段示例 | 常见含义 | 推理时的处理 |
|---|---|---|
| `BH` | 编号/台账序号 | 待核实项目或地块编号线索；**不能**单独当作项目名称 |
| `SGQK` | 施工情况 | 建设状态线索；与 POI/公示名称组合时价值更高 |
| `XZMC` | 乡镇/行政村名称 | 行政背景；辅助 gov Web 检索 query，**不是**项目名 |
| `Area` / `area` / `面积` | 面积（可能与 MCP `area_m2` 单位不同） | 与几何面积交叉验证；单位不明时只记录原值 |

**多项目集群**：同一 FeatureCollection 内多个相邻地块可能属于同一建设片区或**不同项目**。应**逐地物**给出 `related_projects`，不要因邻近 POI 相同就合并结论。

**test3 式混合批次示例**（地块分属不同项目）：空间聚类不同 → 每地物独立结论；仅一侧有 POI/公示名称时，另一侧不得借用该名称。无直接证据地物：`evidence_type=inferred`，confidence ≤0.4，措辞用「具体项目名称未知」而非邻块项目名。若共享项目名称但编号/状态不同，在 evidence 中说明空间关系与不确定性。
