...
"""CLI wrapper for Baidu place search (+ conditional regeo via geo_clients)."""
import json
import os
import sys
...
from geo_core.clients import maybe_regeo_baidu, query_baidu  # noqa: E402
...
def main():
    ...
    while i < len(rest):
        if rest[i] == "--timeout" and i + 1 < len(rest):
            ...
            i += 1
    result = maybe_regeo_baidu(query_baidu(lat, lon, radius_m), lat, lon)
